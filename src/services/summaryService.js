...
const normalizarCodigos = (codigos) =>
  (Array.isArray(codigos) ? codigos : []).map((c) => String(c || "").trim());
...
async function obtenerResumen({
  ...
  const comp =
    anioComparativo != null ? Number(anioComparativo) : ejercicio - 1;
  ...
  console.log(
    `📅 Año actual: ${ejercicio}, Comparativo: ${comp}, Periodo: ${periodoNum}`
  );
  ...
  const mapaPresupuestosActual = new Map(
    presupuestosActual.map((r) => [r.cuenta, r])
  );
  const mapaPresupuestosComp = new Map(
    presupuestosComp.map((r) => [r.cuenta, r])
  );

  // Consulta ejercicio actual
  const selActual = construirSelectResumen({
    anio: ejercicio,
    periodo: periodoNum,
  ...
  console.log(
    `📊 [SUMMARY] Consultando ejercicio ${ejercicio} para empresa ${empresaId}`
  );
  console.log(`📋 [SUMMARY] SQL: ${selActual.sql.substring(0, 200)}...`);
  console.log(
    `🔢 [SUMMARY] Parámetros: ${JSON.stringify(selActual.parametros)}`
  );

  let filasActual = [];
  try {
    filasActual = await ejecutarConsulta(
      empresaId,
      selActual.sql,
      selActual.parametros
    );
    console.log(
      `✅ [SUMMARY] Filas obtenidas para ${ejercicio}: ${filasActual.length}`
    );
  } catch (error) {
    console.error(
      `❌ [SUMMARY] Error obteniendo datos ejercicio actual ${ejercicio}:`,
      error.message
    );
    console.warn(
      `⚠️ [SUMMARY] Se continuará con 0 filas para ${ejercicio} para permitir mostrar comparativo.`
    );
    filasActual = [];
  }
  ...
  if (filasActual.length > 0) {
    ...
    const filasConDatos = filasActual.filter(
      (f) => f.MES !== 0 || f.YTD !== 0
    ).length;
    console.log(
      `📊 [SUMMARY] Filas con datos (MES o YTD != 0): ${filasConDatos}/${filasActual.length}`
    );

    if (filasConDatos === 0) {
      console.warn(
        `⚠️ [SUMMARY] TODAS LAS FILAS ESTÁN EN CEROS para año ${ejercicio}`
      );
      console.warn(`⚠️ [SUMMARY] Posibles causas:`);
      console.warn(
        `   1. Los códigos de cuenta no coinciden con la base (formato diferente)`
      );
      console.warn(
        `   2. La tabla SALDOS${anio
          .toString()
          .slice(-2)} no tiene EJERCICIO = ${ejercicio}`
      );
      console.warn(
        `   3. Los códigos de cuenta no existen en la base de datos`
      );
    }
  } else {
  ...
  const selComp = construirSelectResumen({
    anio: comp,
    periodo: periodoNum,
    usarAjusteEnYTD: usarAjuste,
    codigos: listaCodigos,
  ...
  console.log(
    `📊 [SUMMARY] Consultando ejercicio comparativo ${comp} para empresa ${empresaId}`
  );
  const filasComp = await ejecutarConsulta(
    empresaId,
    selComp.sql,
    selComp.parametros
  );
  ...
  const todasEnCeros =
    filasActual.filter((f) => f.MES !== 0 || f.YTD !== 0).length === 0;
  if (todasEnCeros && filasActual.length > 0) {
    console.log(
      `\n🔍 [DIAGNÓSTICO] Verificando cuentas existentes en CUENTAS${ejercicio
        .toString()
        .slice(-2)}...`
    );
    try {
      ...
      console.log(
        `📋 [DIAGNÓSTICO] Cuentas que SÍ existen en la base (primeras 10):`
      );
      cuentasReales.forEach((c, i) => {
        console.log(
          `   ${i + 1}. ${c.NUM_CTA} - ${c.NOMBRE?.substring(0, 40)}`
        );
      });
      ...
      primerosCodigosSolicitados.forEach((codigo) => {
        ...
        console.log(
          `   ${codigo}: ${existe ? "✅ EXISTE" : "❌ NO ENCONTRADO"}`
        );
      });
    } catch (err) {
      console.error(
        `❌ [DIAGNÓSTICO] Error al verificar cuentas:`,
        err.message
      );
    }
  }
  ...
  const mesActualClave =
    periodoNum >= 1 && periodoNum <= 12 ? MESES_CLAVES[periodoNum - 1] : "ene";

  const detalle = listaCodigos.map((codigo) => {
    const a = mapaActual.get(codigo) || {
      codigo,
      descripcion: "",
      mes: 0,
      ytd: 0,
    };
    const b = mapaComp.get(codigo) || {
      codigo,
      descripcion: "",
      mes: 0,
      ytd: 0,
  ...
  const totalAcumulado = detalle.reduce(
    (acc, it) => acc + (Number(it.acumuladoActual) || 0),
    0
  );
  const totalAcumuladoAnterior = detalle.reduce(
    (acc, it) => acc + (Number(it.acumuladoAnterior) || 0),
    0
  );
  ...
  if (Number.isFinite(totalAcumuladoAnterior)) {
    ejercicios.push({
      anio: comp,
      saldo: totalAcumuladoAnterior,
      acumulado: totalAcumuladoAnterior,
    });
  }
