    ...
    try {
      ...
      const resolveByLabel = (tipo, label, parentHint = "") => {
          ...
          if (
            ...
            if (tipoNorm === "section" || tipoNorm === "seccion") {
              ...
              if (principalNode?.children?.length) {
                ...
                if (!totalsAreZero(sumado)) return sumado;
              }
            }
          }
          return bySection;
        ...
        if (
          ...
          if (byOperation) return byOperation;
          // Fallback: sumar children directamente (principal con totales en cero por modo estricto)
          const principalNode = obtenerPrincipalPorEtiqueta(valor);
          if (principalNode?.children?.length) {
            const sumado = principalNode.children.reduce(
              (acc, sec) => ({
...
async function generarReporte(
  ...
  // Aplicar visibilidad por subseccion según layoutConfig.subseccionesOcultas
  const ocultas = Array.isArray(layoutConfig?.subseccionesOcultas)
    ? layoutConfig.subseccionesOcultas
    : [];
  if (ocultas.length > 0) {
    layout.forEach((block) => {
      if (block.type === "secundaria") {
        ...
        if (hidden) block.mostrarEnResumen = false;
      }
    });
  }
