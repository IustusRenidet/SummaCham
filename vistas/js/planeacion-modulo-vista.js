(() => {
  ...
  const WORKFLOW_ETIQUETAS = {
    ...
    REVISADO: {
      texto: "Revisado",
      descripcion: "Listo para autorizar o volver a editar.",
    },
    APROBADO: {
      texto: "Aprobado",
      descripcion: "Listo para guardar en base de datos.",
    },
    GUARDADO: {
      texto: "Guardado",
      descripcion: "Movimiento de guardado en el flujo de autorizacion.",
    },
  ...
  const obtenerPermisosActuales = (sesion, modulo) => {
    ...
    const permisos =
      Sesion.obtenerPermisosModulo(modulo, empresa.id, sesion) || {};
  ...
  const prepararFilasBusqueda = (tabla) => {
    ...
    filas.forEach((fila) => {
      if (
        ...
        fila.classList.contains("result-row")
      ) {
      ...
      const descripcion =
        fila.children[1]?.textContent?.trim().toLowerCase() || "";
  ...
  const obtenerAniosDesdeEndpoint = async (
    empresaId,
    endpointAnios = "saldos"
  ) => {
    ...
    try {
      ...
      const resp = await fetch(
        `${API_BASE}/${endpoint}/anios?${params.toString()}`,
        {
      ...
      if (!resp.ok) {
        ...
        throw new Error(
          datos.mensaje || "No fue posible obtener los ejercicios disponibles."
        );
      }
      return normalizarListaEnteros(datos.anios || []);
    } catch (error) {
      console.warn(
        `No fue posible obtener ejercicios desde ${endpoint}`,
        error
      );
      return [];
    }
  ...
  const initVistaModuloPlaneacion = (config = {}) => {
    ...
    const opciones = {
      endpointAnios: "comites",
      ...obtenerConfigDesdeDataset(),
      ...config,
    };
    opciones.endpointAnios = limpiarEndpoint(opciones.endpointAnios, "comites");
    const nombreArchivoModulo = crearSlug(
      opciones.titulo || opciones.modulo || "modulo"
    );
    const datasetAnio = Number(document.body?.dataset?.anio);
    const ctxAnio = leerContextoPersistido();
    const anioInicial = Number.isInteger(datasetAnio)
      ? datasetAnio
      : ctxAnio ?? new Date().getFullYear();
    ...
    const elementos = {
      ...
      loadBudgetBtn:
        ...
        document.getElementById("loadBudgetBtn"),
      reviewBudgetBtn:
        document.getElementById("btnMarcarRevisado") ||
        document.getElementById("reviewBudgetBtn"),
      authorizeBudgetBtn:
        document.getElementById("btnAutorizar") ||
        document.getElementById("authorizeBudgetBtn"),
    ...
    const notificarContexto = () => {
      ...
      window.dispatchEvent(
        new CustomEvent(EVENTO_CONTEXTO, { detail: detalle })
      );
    };
    ...
    const actualizarYearLabels = () => {
      ...
      if (
        elementos.yearSelect &&
        Number(elementos.yearSelect.value) !== Number(estado.anio)
      ) {
    ...
    const cargarAniosDisponibles = async () => {
      ...
      const aniosDisponibles = await obtenerAniosDesdeEndpoint(
        empresa.id,
        opciones.endpointAnios
      );
    ...
    const aplicarVisibilidadCuentas = () => {
      ...
      if (elementos.toggleAccountsBtn) {
        elementos.toggleAccountsBtn.textContent = estado.mostrarCuentas
          ? "Ocultar cuentas"
          : "Mostrar cuentas";
        elementos.toggleAccountsBtn.setAttribute(
          "aria-pressed",
          estado.mostrarCuentas ? "true" : "false"
        );
      }
    };
    ...
    const actualizarBadgeWorkflow = () => {
      ...
      const info =
        WORKFLOW_ETIQUETAS[estado.workflow.estado] ||
        WORKFLOW_ETIQUETAS.SIN_CARGAR;
      ...
      if (elementos.workflowMeta) {
        ...
        const usuario = estado.workflow.actualizadoPor
          ? ` por ${estado.workflow.actualizadoPor}`
          : "";
        const meta = fecha
          ? `${info.descripcion} Actualizado ${fecha}${usuario}.`
          : info.descripcion;
    ...
    const renderizarHistorial = () => {
      ...
      historial.forEach((registro) => {
        ...
        const etiqueta =
          WORKFLOW_ETIQUETAS[registro.estado]?.texto || registro.estado;
    ...
    const actualizarDisponibilidadAcciones = () => {
      ...
      const puedeCargar =
        esAdminGlobal || Boolean(permisos?.[TRANSICIONES.cargar.permiso]);
      const puedeRevisar =
        esAdminGlobal || Boolean(permisos?.[TRANSICIONES.revisar.permiso]);
      const puedeAutorizar =
        esAdminGlobal || Boolean(permisos?.[TRANSICIONES.autorizar.permiso]);
      const puedeGuardar =
        esAdminGlobal || Boolean(permisos?.[TRANSICIONES.guardar.permiso]);
      ...
      if (elementos.reviewBudgetBtn) {
        elementos.reviewBudgetBtn.classList.toggle(
          "d-none",
          esAdminGlobal || !puedeRevisar
        );
        elementos.reviewBudgetBtn.disabled =
          esAdminGlobal ||
          !puedeRevisar ||
          !(
            esAdminGlobal ||
            (requiereGuardado && TRANSICIONES.revisar.habilita(estadoActual))
          );
      }
      if (elementos.authorizeBudgetBtn) {
        elementos.authorizeBudgetBtn.classList.toggle(
          "d-none",
          esAdminGlobal || !puedeAutorizar
        );
        elementos.authorizeBudgetBtn.disabled =
          esAdminGlobal ||
          !puedeAutorizar ||
          !(
            esAdminGlobal ||
            (requiereGuardado && TRANSICIONES.autorizar.habilita(estadoActual))
          );
      }
      if (elementos.saveBudgetBtn) {
        const visibleGuardar =
          esAdminGlobal || (requiereGuardado && estadoActual === "APROBADO");
        elementos.saveBudgetBtn.classList.toggle(
          "d-none",
          !puedeGuardar || !visibleGuardar
        );
        elementos.saveBudgetBtn.disabled =
          !puedeGuardar ||
          !visibleGuardar ||
          !(
            esAdminGlobal ||
            (requiereGuardado && TRANSICIONES.guardar.habilita(estadoActual))
          );
      }
    };
    ...
    const obtenerWorkflow = async () => {
      ...
      try {
        ...
        const respuesta = await fetch(
          `${API_BASE}/presupuestos/estado?${params.toString()}`,
          {
            headers: {
        ...
        console.warn(
          `Error al obtener el workflow de ${opciones.titulo}`,
          error
        );
        showToast(
          error.message || "No se pudo actualizar el estado del flujo.",
          "text-bg-danger"
        );
        estado.workflow = {
          estado: "SIN_CARGAR",
    ...
    const ejecutarAccionWorkflow = async (accion) => {
      ...
      if (!Number.isInteger(estado.anio)) {
        showToast(
          "Selecciona un ejercicio válido antes de continuar.",
          "text-bg-warning"
        );
        return;
      }
      ...
      if (!Sesion.esAdminGlobal(sesion) && requiereGuardado.has(accion)) {
        if (estado.editMode) {
          showToast(
            "Finaliza la edición (Cancelar) antes de continuar.",
            "text-bg-warning"
          );
          return;
        }
        if (!estado.borradorGuardado) {
          showToast(
            "Guarda el borrador antes de avanzar en el flujo.",
            "text-bg-warning"
          );
          return;
        }
      ...
      try {
        ...
        if (!respuesta.ok) {
          ...
          throw new Error(
            datos.mensaje || "No fue posible actualizar el flujo."
          );
        }
        const datos = await respuesta.json();
        estado.workflow = {
        ...
        console.error(
          `Error al ejecutar la transición del workflow de ${opciones.titulo}`,
          error
        );
        showToast(
    ...
    const guardarPresupuestoEnBD = async () => {
      if (workflowControlExterno) {
        throw new Error(
          "El guardado en base de datos es manejado por el nuevo flujo de autorizacion."
        );
      }
      const empresa = Sesion.obtenerEmpresaActiva(sesion);
      const anioSeleccionado = Number.isInteger(estado.anio)
        ? estado.anio
        : null;
      if (!empresa?.id || !anioSeleccionado) {
        throw new Error(
          "Selecciona una empresa y ejercicio válidos antes de guardar."
        );
      }
      ...
      if (!respuesta.ok) {
        throw new Error(
          datos.mensaje ||
            "No fue posible guardar los datos en la base de datos."
        );
      }
      return datos;
    };
    ...
    if (!workflowControlExterno && elementos.reviewBudgetBtn) {
      elementos.reviewBudgetBtn.addEventListener("click", () =>
        ejecutarAccionWorkflow("revisar")
      );
    }

    if (!workflowControlExterno && elementos.authorizeBudgetBtn) {
      elementos.authorizeBudgetBtn.addEventListener("click", () =>
        ejecutarAccionWorkflow("autorizar")
      );
    }
    ...
    window.addEventListener(
      "modulo-planeacion:presupuesto-editado",
      (event) => {
    ...
    if (!workflowControlExterno && elementos.saveBudgetBtn) {
      elementos.saveBudgetBtn.addEventListener("click", async () => {
        try {
          ...
          showToast(
            respuesta.mensaje || "Datos guardados en la base de datos."
          );
  ...
  (() => {
    ...
    // Nota: El diagnóstico de bloqueadores solo se ejecuta una vez al cargar
    // para evitar logs excesivos. Usar window.__diagnosticarBloqueadores() manualmente si se necesita.
