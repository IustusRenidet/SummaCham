...
const MODULE_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'comites', label: 'ComitÈs', path: 'ComitÈs.html', permiso: 'ComitÈs' },
      { id: 'comunicacion', label: 'ComunicaciÛn', path: 'ComunicaciÛn.html', permiso: 'ComunicaciÛn' },
      { id: 'direccion', label: 'DirecciÛn', path: 'DirecciÛn.html', permiso: 'DirecciÛn' },
      { id: 'eventos', label: 'Eventos', path: 'Eventos.html', permiso: 'Eventos' },
      {
        ...
        items: [
          ...
          { id: 'nomina', label: 'NÛmina', path: 'Nomina.html', permiso: 'NÛmina' }
        ]
      },
      { id: 'gtos-corporativos', label: 'Gastos Corporativos', path: 'Gtos_Corporativos.html', permiso: 'Gtos Corporativos' },
      { id: 'membresia', label: 'MembresÌa', path: 'MembresÌa.html', permiso: 'MembresÌa' },
      { id: 'rh', label: 'Recursos Humanos', path: 'RH.html', permiso: 'RH' },
      { id: 'serv-membresia', label: 'Servicios a la MembresÌa', path: 'Serv_MembresÌa.html', permiso: 'Serv MembresÌa' },
...
const usuarioPuedeUsarModulo = (sesion, empresaId, modulo, puedeAdministrar) => {
  ...
  // Incluir permiso "Ver" (puede_leer) para mostrar el m√≥dulo
...
const LoginView = ({ onLogin }) => {
  ...
  const manejarEnvio = async (evento) => {
    ...
    try {
      ...
      if (!respuesta.ok) {
        throw new Error(datos.mensaje || 'No fue posible iniciar sesi√≥n.');
      }
      const sesionNormalizada = Sesion.guardar(datos) || datos;
      onLogin(sesionNormalizada);
    } catch (err) {
      console.error('Error de inicio de sesi√≥n', err);
      setError(err.message || 'Ocurri√≥ un problema durante el inicio de sesi√≥n.');
    } finally {
      setCargando(false);
    }
  };
    ...
    <div className="d-flex align-items-center justify-content-center h-100">
      <main className="login-card" aria-labelledby="titulo-login">
        ...
        <form onSubmit={manejarEnvio} autoComplete="off">
          ...
          <div className="mb-3">
            <label htmlFor="contrasena" className="form-label fw-semibold">Contrase√±a <span className="text-danger">*</span></label>
...
const DashboardLayout = ({
  ...
  return (
    <div className={layoutClassName}>
      <aside className="app-sidebar" aria-label="Navegaci√≥n principal">
        ...
        <footer className="sidebar-footer">
          <div className="user-info mb-3">
            <span className="text-muted text-uppercase fw-semibold" style={{ letterSpacing: '0.08em', fontSize: '0.7rem' }}>Sesi√≥n activa</span>
            <strong>{obtenerNombreUsuario(sesion)}</strong>
            <span className="text-muted" style={{ fontSize: '0.9rem' }}>{sesion?.usuario?.usuario || '‚Äî'}</span>
          </div>
        ...
        <div className="top-bar">
          <div className="top-bar-left">
            <button
              ...
              aria-label={sidebarOculta ? 'Mostrar men√∫ lateral' : 'Ocultar men√∫ lateral'}
              aria-expanded={!sidebarOculta}
            >
              <span aria-hidden="true">‚ò∞</span>
            </button>
            <div>
              <h2>{moduloSeleccionado ? moduloSeleccionado.label : 'Selecciona un m√≥dulo'}</h2>
...
const App = () => {
  ...
  const marcarNotificacion = useCallback(async (id) => {
    ...
    try {
      ...
      if (!respuesta.ok) {
        const datos = await respuesta.json();
        throw new Error(datos.mensaje || 'No fue posible actualizar la notificaci√≥n.');
      }
      cargarNotificaciones();
    } catch (error) {
      console.warn('Error al marcar notificaci√≥n como le√≠da', error);
    }
  }, [cargarNotificaciones]);
