...
(() => {
  ...
  // Restringir a plantillas.html
  if (!window.location.pathname.includes("plantillas.html")) {
    // Definir un objeto vacío para evitar errores por si otros scripts lo llaman,
    // pero sin funcionalidad real.
    window.InsertionWizard = {
      open: () => console.log("Wizard deshabilitado en este módulo"),
    };
    return;
  }
  ...
  const InsertionWizard = {
    ...
    operacionesSumasTipos: [
      ...
      {
        key: "sum-row-operativo-consolidado",
        label: "Sum Row (operativo consolidado)",
      },
    ...
    open(referenceRow = null, options = {}) {
      if (!this.isEditModeAllowed()) {
        alert(
          "❌ Debes activar el modo edición antes de usar el wizard de inserción."
        );
        return;
      }
      ...
      if (
        prefillContext?.operaciones &&
        typeof prefillContext.operaciones === "object"
      ) {
        this.formData.operaciones = { ...prefillContext.operaciones };
      }
      if (Array.isArray(prefillContext?.customOperaciones)) {
        this.formData.customOperaciones = prefillContext.customOperaciones.map(
          (op) => ({
            key: op.key || "",
            label: op.label || "",
          })
    ...
    isEditModeAllowed() {
      ...
      const modoEdicionApi =
        window.ModoEdicionPresupuesto || window.parent?.ModoEdicionPresupuesto;
      let modoEdicion = false;

      // Verificar modo edición via FlujoAutorizacion
      if (flujo) {
        modoEdicion = Boolean(flujo.state?.editMode);
      }

      // Verificar modo edición via ModoEdicionPresupuesto
      if (!modoEdicion && modoEdicionApi?.estaActivo) {
        modoEdicion = modoEdicionApi.estaActivo();
      }

      // Si no hay ningún sistema de edición disponible, no permitir
      if (!flujo && !modoEdicionApi) {
        console.warn("[InsertionWizard] No hay sistema de edición disponible");
        return false;
      }

      console.log("[InsertionWizard] Modo edición:", modoEdicion);
      return modoEdicion;
    },
    ...
    getCurrentCapitulo() {
      const mapearCapitulo = (empresaId) => {
        ...
        if (
          ...
          typeof window.CapitulosModulos.empresaACapitulo === "function"
        ) {
          const capitulo = window.CapitulosModulos.empresaACapitulo(empresaId);
          if (capitulo) {
            console.log(
              "? Cap¡tulo detectado via CapitulosModulos:",
              empresaId,
              "→",
              capitulo
            );
      ...
      if (selectorActivo) {
        ...
        console.log(
          "?? Usando selector visible para cap¡tulo:",
          selectedText || empresaId
        );
      ...
      console.warn(
        "?? No se encontr¢ selector global de empresa (companyFilter)"
      );
      return "";
    },
            ...
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title">
                  ...
                  Agregar Elemento - Paso ${this.currentStep} de ${
        this.totalSteps
      }
                </h5>
            ...
            ${options
              ...
              <label class="list-group-item list-group-item-action d-flex align-items-center">
                <input class="form-check-input me-3" type="radio" name="elementType" 
                       value="${opt.value}" ${
                  this.selectedType === opt.value ? "checked" : ""
                }
    ...
    async loadAllOptions() {
      ...
      const hierarchy = rules.hierarchy
        ? rules.hierarchy.filter((level) => level !== "capitulo")
        : [];
    ...
    async loadDependentOptions(changedLevel) {
      ...
      const hierarchy = (rules.hierarchy || []).filter(
        (level) => level !== "capitulo"
      );
    ...
    renderStep3_EnterData() {
      const rules = this.getValidationRules()[this.selectedType];
      if (!rules) return "<p>Error: Tipo no válido</p>";
      const factorNumerico = Number.isFinite(Number(this.formData.factor))
        ? Number(this.formData.factor)
        : null;
      const factorChoice =
        ...
        (factorNumerico === null
          ...
          ? "1"
          : "custom");
      const factorCustomValor =
        factorChoice === "custom" ? this.formData.factor ?? "" : "";
      ...
      const signoActual = Number.isFinite(Number(this.formData.signo))
        ? Number(this.formData.signo)
        : 1;
      ...
      return `
        <div class="wizard-step active" data-step="3">
          <h6 class="mb-3">Datos del ${this.getTypeLabel(
            this.selectedType
          )}</h6>
          ...
          ${
            ...
            <div class="mb-3">
              ...
              <div class="row g-2">
                ${this.operacionesSumasTipos
                  .map(
                    (op) => `
                  <div class="col-12 col-md-6">
                    <label class="form-label small text-muted mb-1" for="data_op_${
                      op.key
                    }">
                      ${op.label}
                    </label>
                    <input type="text" class="form-control" id="data_op_${
                      op.key
                    }"
                           value="${operacionesActuales[op.key] || ""}"
                           placeholder="Etiqueta (ej: NET RESULTS)"
                           oninput="InsertionWizard.updateOperacionSumas('${
                             op.key
                           }', this.value)">
                  </div>
                `
      ...
      const types = {
        SUMMARY: [
          {
            ...
            icon: "📊",
            description:
              "Agregar una cuenta contable a una operación existente",
          },
          {
            value: "operacion",
          ...
          },
          {
            ...
            icon: "➗",
            description:
              "Definir SUMA DE VARIAS SECCIONES (sumavarios, net, operativo)",
          },
        ],
        RESUMEN: [
          {
        ...
        MODULOS: [
          ...
          },
          {
            ...
            icon: "⚙️",
            description:
              "Agrupar cuentas en una operación dentro de una sección",
          },
          {
            value: "seccion",
            label: "Nueva Sección",
    ...
    async getOptionsForLevel(level) {
      ...
      try {
        ...
        if (this.contextData.capitulo)
          params.append("capitulo", this.contextData.capitulo);
        if (this.contextData.anio) params.append("anio", this.contextData.anio);

        // Agregar jerarquía para filtrado
        if (this.contextData.principal)
          params.append("principal", this.contextData.principal);
        if (this.contextData.secundaria)
          params.append("secundaria", this.contextData.secundaria);

        const response = await fetch(
          `/api/insercion/opciones/${level}?${params}`,
          {
    ...
    addCustomOperacionSumas() {
      if (!this.formData.customOperaciones)
        this.formData.customOperaciones = [];
    ...
    updatePreview() {
      ...
      if (this.currentStep === 3 && tieneDatosPreview) {
        ...
        const hierarchy = [];
        if (this.contextData.capitulo)
          hierarchy.push(this.contextData.capitulo);
        if (this.contextData.principal)
          hierarchy.push(this.contextData.principal);
        if (this.contextData.secundaria)
          hierarchy.push(this.contextData.secundaria);
        if (this.contextData.operacion)
          hierarchy.push(this.contextData.operacion);
        ...
        const nombrePreview =
          ...
          "";
        content.innerHTML = `
          <div><strong>${this.getTypeLabel(
            this.selectedType
          )}:</strong> ${nombrePreview}</div>
          ${
            hierarchy.length > 0
              ? `<div class="text-muted">📍 ${hierarchy.join(" > ")}</div>`
              : ""
          }
          ${
            factorLabel
              ? `<div class="text-info">Operaci¢n: ${factorLabel}</div>`
              : ""
          }
          ${
            this.formData.etiquetaSum
              ? `<div class="text-success">✓ Se creará SUM ROW: "${this.formData.etiquetaSum}"</div>`
              : ""
          }
          ${
            this.selectedType === "operacion_sumas"
            ...
            <div class="text-info">Sección: ${
              this.formData.seccion || this.contextData.secundaria || ""
            }</div>
            ${
              operacionesTexto
                ? `<div class="text-muted small">Operaciones: ${operacionesTexto}</div>`
                : ""
            }
          `
              : ""
        ...
        case 3:
          ...
          return requiredFields.every((field) => {
            if (this.selectedType === "operacion_sumas") {
              ...
              const tieneOperacion = Object.values(operaciones).some(
                (value) => value
              );
              if (!tieneOperacion) return false;
              if (
                !this.formData.clase ||
                this.formData.clase.toString().trim() === ""
              )
                return false;
              const seccion =
                this.formData.seccion || this.contextData.secundaria;
    ...
    async submit() {
      ...
      if (window.InsertionValidator) {
        ...
        const validationResult =
          window.InsertionValidator.validarAntesDeProcesar(validationData);
    ...
    async realizarInsercion() {
      ...
      if (this.selectedType === "cuenta") {
        ...
        ["secundaria", "principal", "seccion"].includes(this.selectedType)
      ) {
    ...
    buildInsertionData() {
      ...
      const base = {
        ...
        formData: {
          ...
          operacionFactor: Number.isFinite(Number(this.formData.factor))
            ? Number(this.formData.factor)
            : 1,
          factor: Number.isFinite(Number(this.formData.factor))
            ? Number(this.formData.factor)
            : 1,
        },
      };
      ...
      if (this.selectedType === "operacion_sumas") {
        base.formData = {
          ...
          signo: Number.isFinite(Number(this.formData.signo))
            ? Number(this.formData.signo)
            : 1,
    ...
    async insertarOperacionSumas(data) {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(
          error.mensaje || "Error al insertar operación entre filas"
        );
      }
